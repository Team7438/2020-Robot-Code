...
public class DriveSub extends Subsystem {
  ...
  public static AHRS Gyro = new AHRS(SPI.Port.kMXP);
    ...
    public static void arcadeDrive(final double xSpeed, final double zRotation) {
    if (!disableDrive) {
  ...
  public void robotDriver(final Joystick joystickZero) {
    // Experimental throttle curve stuff -- Originall arcadeDeive line is below
    // (commented out)
    ...
    // This next line overrides all the other stuff and feeds straight joystic
    // values.
    ...
    // DriveBase.arcadeDrive(squareInput(-joystickZero.getY())/3,
    // squareInput(joystickZero.getZ())/3);
    // centerMotor.setSpeed(-joystickZero.getX()/2);
  }

  public double squareInput(final double input) {
    if (input < 0) {
      return -(input * input);
    } else {
      return (input * input);
  ...
  public void periodic() {
    ...
    // m_odometry.update(Rotation2d.fromDegrees(getHeading()), m_encoderLeft.getDistance(), m_encoderRight.getDistance());
  }
  ...
  public void driveForward(final double speed) {
    final double strturningValue = (0 - Gyro.getAngle()) * kP;
  ...
  public void driveBackward(final double speed) {
    final double strturningValue = (0 - Gyro.getAngle()) * kP;
    // Invert the direction of the turn if we are going backwards
    // turningValue = Math.copySign(turningValue, speed);
