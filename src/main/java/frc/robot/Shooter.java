...
public class Shooter {
    ...
    int displayed_frame_h_width  = -1;  // 320/640/960
    int displayed_frame_h_height = -1;  // 240/360/540
    ...
    */
    ...
    calibDist = camera_id == 1 ? 2.0f : 2.7f;  // meters
    calibBoundingWidth = camera_id == 1 ? 310.0f : 370f;
    ...
    private void annotateFrame(Mat source) {
        ...
        // targetPose[0] in NT is distance to the target
        ...
        // shooterSight is the point on the screen where the shooter axis intersects image plane
        ...
        double velocity = 20.0f;  // TODO get from NT
        Point ballisticTarget = balistic_drop(shooterSight, distance, velocity, shooterPitch);  // TODO normalize units

        // draw the ballistic target (where the ball woudl hit the image plane)
        if(targetVisible)
        ...
        aimShooter(shooterSight, distance, velocity, shooterPitch);

    }

    private Point balistic_drop(Point shooterTarget, double target_distance, double velocity, double pitch) {
   
        Point ballisticTarget = new Point();
        ballisticTarget.x = shooterTarget.x;
        ballisticTarget.y = shooterTarget.y;

        /* enable ballistic calculation
        double x = target_distance*Math.sin(Math.PI*pitch/180.0f);  // horizontal distance
        ...
        */
        ...
        if (HD_format) {
            ballisticTarget.y += 150;  // harcoded ballistic offset
        } else {
            ballisticTarget.y += 150;  // harcoded ballistic offset
        }
    ...
    public void aimShooter(Point shooterTarget, double target_distance, double velocity, double pitch) {

        /*
        while(true) {
            balistic_drop(shooterTarget, target_distance, velocity, pitch);
        }
        */
